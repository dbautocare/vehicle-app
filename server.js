...
function analyseMotHistory(motData) {
    const result = {
        ...
        motRiskNotes: [],
        mileageAnomalies: [],
        motCategories: { corrosion: 0, tyres: 0, brakes: 0, suspension: 0, lights: 0, emissions: 0, generalNeglect: 0 },
    ...
    tests.forEach(test => {
        ...
        if (test.defects && test.defects.length > 0) {
            test.defects.forEach(defect => {
                ...
                if (text.includes("corrosion") || text.includes("rust")) { result.motCategories.corrosion++; result.motRiskScore += 10; }
                if (text.includes("tyre") || text.includes("tread")) { result.motCategories.tyres++; result.motRiskScore += 3; }
                if (text.includes("brake") || text.includes("disc") || text.includes("pad")) { result.motCategories.brakes++; result.motRiskScore += 4; }
                if (text.includes("suspension") || text.includes("spring") || text.includes("shock") || text.includes("bush")) { result.motCategories.suspension++; result.motRiskScore += 5; }
                if (text.includes("lamp") || text.includes("light") || text.includes("headlamp")) { result.motCategories.lights++; result.motRiskScore += 2; }
                if (text.includes("emission") || text.includes("smoke")) { result.motCategories.emissions++; result.motRiskScore += 8; }
            });
        }
    });

    if (failCount >= 3) { result.motRiskNotes.push("Multiple MOT failures recorded"); result.likelyRetailSpeedScore -= 10; }
    if (advisoryCount >= 8) { result.motRiskNotes.push("High number of advisories"); result.likelyRetailSpeedScore -= 8; }
    if (result.motCategories.corrosion > 0) { result.motRiskNotes.push("Corrosion history found"); result.likelyRetailSpeedScore -= 15; }
    if (result.motCategories.tyres >= 3) { result.motRiskNotes.push("Repeated tyre advisories suggest maintenance neglect"); result.likelyRetailSpeedScore -= 5; }
    if (result.motCategories.brakes >= 3) { result.motRiskNotes.push("Repeated brake wear advisories"); result.likelyRetailSpeedScore -= 5; }
    ...
    if (result.mileageAnomalies.length === 0) result.motRiskNotes.push("No mileage anomalies detected");
...
function buildNegotiationHelper({ motAnalysis, specAnalysis, motLength, condition, prepCosts, mileage }) {
    ...
    if (prepCosts?.length > 0) points.push(`Use estimated prep costs of £${prepCosts.reduce((a, b) => a + b.cost, 0)} during negotiation.`);
    if (points.length === 0) points.push("No major negotiation weaknesses found. Focus on market price and margin.");
    return points;
}
...
app.post("/vehicle", async (req, res) => {
    try {
        const { registration, vin, mileage, condition, spec, serviceHistory, bodywork, tyres, alloys, interior, warningLights, keys, v5, motLength, owners, gearbox, clutch, notes } = req.body;
        ...
        const vehicle = {
            ...
            year: motData?.firstUsedDate ? new Date(motData.firstUsedDate).getFullYear() : "Unknown",
            fuel: motData?.fuelType || "Unknown",
            colour: motData?.primaryColour || "Unknown",
            engine: "Unknown"
        };

        const marketListings = [10995, 11250, 10495, 11495, 10695, 10895, 11195, 10750];
        let marketAverage = marketListings.reduce((a, b) => a + b, 0) / marketListings.length;
        let adjustments = [];
        let prepCosts = [];
        ...
        if (specAnalysis.specValueAdjustment > 0) adjustments.push(`Desirable spec adjustment: +£${specAnalysis.specValueAdjustment}`);
        ...
        if (motAnalysis.motRiskLevel === "Medium") { marketAverage -= 350; desirabilityScore -= 6; buyScore -= 8; prepCosts.push({ item: "MOT risk allowance", cost: 250 }); adjustments.push("Medium MOT risk: -£350"); }
        if (motAnalysis.motRiskLevel === "High") { marketAverage -= 900; desirabilityScore -= 15; buyScore -= 20; prepCosts.push({ item: "High MOT risk allowance", cost: 700 }); adjustments.push("High MOT risk: -£900"); }
        if (motAnalysis.mileageAnomalies.length > 0) { marketAverage -= 1200; desirabilityScore -= 25; buyScore -= 30; adjustments.push("Mileage anomaly detected: -£1200"); }
        if (motAnalysis.motCategories.corrosion > 0) { marketAverage -= 750; desirabilityScore -= 12; buyScore -= 15; prepCosts.push({ item: "Corrosion risk allowance", cost: 600 }); adjustments.push("Corrosion history found: -£750"); }

        if (condition === "poor") { marketAverage -= 1500; desirabilityScore -= 15; buyScore -= 18; prepCosts.push({ item: "General reconditioning", cost: 800 }); adjustments.push("Poor overall condition: -£1500"); }
        if (condition === "average") { marketAverage -= 750; desirabilityScore -= 6; buyScore -= 8; prepCosts.push({ item: "General preparation", cost: 350 }); adjustments.push("Average overall condition: -£750"); }
        if (condition === "excellent") { marketAverage += 1000; desirabilityScore += 10; buyScore += 10; adjustments.push("Excellent overall condition: +£1000"); }

        if (serviceHistory === "full") { marketAverage += 500; desirabilityScore += 8; buyScore += 8; adjustments.push("Full service history: +£500"); }
        if (serviceHistory === "partial") { marketAverage -= 250; desirabilityScore -= 4; buyScore -= 4; adjustments.push("Partial service history: -£250"); }
        if (serviceHistory === "none") { marketAverage -= 750; desirabilityScore -= 12; buyScore -= 12; prepCosts.push({ item: "Service due / history risk", cost: 250 }); adjustments.push("No service history: -£750"); }

        if (bodywork === "poor") { marketAverage -= 1000; desirabilityScore -= 12; buyScore -= 14; prepCosts.push({ item: "Bodywork repairs", cost: 800 }); adjustments.push("Poor bodywork: -£1000"); }
        if (bodywork === "average") { marketAverage -= 400; desirabilityScore -= 5; buyScore -= 5; prepCosts.push({ item: "Minor bodywork", cost: 300 }); adjustments.push("Average bodywork: -£400"); }
        if (tyres === "poor") { marketAverage -= 400; desirabilityScore -= 5; buyScore -= 6; prepCosts.push({ item: "Tyres", cost: 350 }); adjustments.push("Poor tyres: -£400"); }
        if (alloys === "poor") { marketAverage -= 350; desirabilityScore -= 4; buyScore -= 4; prepCosts.push({ item: "Alloy refurbishment", cost: 300 }); adjustments.push("Poor alloys: -£350"); }
        if (interior === "poor") { marketAverage -= 600; desirabilityScore -= 8; buyScore -= 8; prepCosts.push({ item: "Interior clean/repair", cost: 250 }); adjustments.push("Poor interior: -£600"); }
        if (warningLights === "yes") { marketAverage -= 1000; desirabilityScore -= 15; buyScore -= 20; prepCosts.push({ item: "Diagnostic/repair allowance", cost: 700 }); adjustments.push("Warning lights present: -£1000"); }
        if (keys === "one") { marketAverage -= 250; desirabilityScore -= 3; buyScore -= 3; prepCosts.push({ item: "Spare key", cost: 220 }); adjustments.push("Only one key: -£250"); }
        if (v5 === "no") { marketAverage -= 500; desirabilityScore -= 8; buyScore -= 10; adjustments.push("No V5 present: -£500"); }
        if (motLength === "short") { marketAverage -= 500; desirabilityScore -= 6; buyScore -= 8; prepCosts.push({ item: "MOT/pre-MOT allowance", cost: 250 }); adjustments.push("Short MOT: -£500"); }
        if (motLength === "none") { marketAverage -= 900; desirabilityScore -= 12; buyScore -= 18; prepCosts.push({ item: "MOT and repair allowance", cost: 600 }); adjustments.push("No MOT: -£900"); }
        if (ownersNumber >= 5) { marketAverage -= 500; desirabilityScore -= 7; buyScore -= 7; adjustments.push("High number of owners: -£500"); }
        if (gearbox === "issue") { marketAverage -= 1500; desirabilityScore -= 18; buyScore -= 25; prepCosts.push({ item: "Gearbox issue allowance", cost: 1200 }); adjustments.push("Gearbox issue: -£1500"); }
        if (clutch === "issue") { marketAverage -= 800; desirabilityScore -= 10; buyScore -= 15; prepCosts.push({ item: "Clutch issue allowance", cost: 650 }); adjustments.push("Clutch issue: -£800"); }

        if (mileageNumber > 100000) { desirabilityScore -= 10; buyScore -= 8; }
        if (mileageNumber < 50000) { desirabilityScore += 8; buyScore += 6; }
        ...
        const vehicleGrade = gradeVehicle({ condition, motAnalysis, buyScore, desirabilityScore });
        const negotiationHelper = buildNegotiationHelper({ motAnalysis, specAnalysis, motLength, condition, prepCosts, mileage });
        ...
        res.json({
            vehicle, registration, marketListings, marketAverage, retailValue, tradeValue,
            adjustments, prepCosts, estimatedPrepTotal, desirabilityScore, buyScore, buyAdvice,
            commonProblems, notes, motData, motAnalysis, specAnalysis, vehicleGrade,
            negotiationHelper, vinDecode
        });
